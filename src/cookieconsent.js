...
(function () {
  ...
  var CookieConsent = function (root) {
    ...
    var _setConfig = function (_user_config) {
      ...
      if (user_config["mode"] === OPT_OUT) _config.mode = OPT_OUT;
    ...
    var _createSettingsModal = function (lang) {
      ...
      for (var i = 0; i < n_blocks; ++i) {
        var title_data = all_blocks[i]["title"],
          ...
          isExpandable =
            !!description_data ||
            (!remove_cookie_tables && !!cookie_table_data);
    ...
    var _saveCookiePreferences = function (accepted_categories) {
      ...
      if (invalid_consent) {
        ...
        if (isFunction(onAccept)) onAccept(saved_cookie_content);
    ...
    var _handleFocusTrap = function () {
      _addEvent(document, "keydown", function (e) {
        ...
        if (focusable_edges) {
          ...
          if (e.shiftKey) {
            if (
              activeElement === focusable_edges[0] ||
              !focused_modal.contains(activeElement)
            ) {
              e.preventDefault();
            ...
            if (
              activeElement === focusable_edges[1] ||
              !focused_modal.contains(activeElement)
            ) {
              e.preventDefault();
